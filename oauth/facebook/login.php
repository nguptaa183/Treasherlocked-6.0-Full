...
if (isset($_GET['code'])) {
	...
	if ( $facebook->IsAuthenticated() ) {
		...
		if ($user_id) {
			...
			$tempUser['first_name'] = ( isset($fb_user['name'])) ? $fb_user['name'] : "User";
			...
			$tempUser['gender'] = (isset ($fb_user['gender']) ) ? (( $fb_user['gender'] == 'male' ) ? 1 : 0) : null;
			
			if ( isset($fb_user['location']) ) {
				$tempUser['location'] = $fb_user['location']->name;
				$_SESSION['location_received'] = true;
			} else {
				$tempUser['location'] = "Not Applicable";
